                  ...
                  Column(
                    ...
                    children: [
                      Text(
                        DateFormat('yMMMMEEEEd', 'fr').format(
                            DateTime.fromMillisecondsSinceEpoch(
                                widget.dateList[0]['start_date'] * 1000)),
                      ...
                      Row(
                        children: [
                          Text(
                            widget.dateList.length > 1
                                ? "${widget.dateList.length} horaires"
                                : "${widget.dateList.length} horaire",
                            style: const TextStyle(
                                fontSize: 12,
                                fontWeight: FontWeight.bold,
                                fontFamily: 'Poppins',
                    ...
                    Wrap(
                      ...
                      children: [
                        for (int i = 0; i < widget.dateList.length; i++)
                          CustomModifHour(
                            selected:
                                widget.selected["first"] == widget.number &&
                                        widget.selected["second"] == i
                                    ? true
                                    : false,
                            id: i,
                            onTap: () => widget.updateFunc(widget.number, i),
                            info: widget.dateList[i],
                          ),
                      ],
                    )
                  ],
